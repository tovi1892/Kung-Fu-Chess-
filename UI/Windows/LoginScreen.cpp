...
LRESULT CALLBACK LoginWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    switch (msg) {
        ...
        case WM_DESTROY: {
            ...
            // WM_QUIT - main.cpp goes on to show further windows (LoginResultScreen, and
            // later RoomChoiceScreen/PlayConfirmScreen) on this same thread, and a WM_QUIT
            // posted here would sit unconsumed (this loop already stops via state->done
            // before calling GetMessage again) and poison the *next* window's first
            // GetMessage call instead, making it return immediately without ever pumping
            // that new window's messages.
            auto* state = stateFor(hwnd);
            if (state) {
                state->done = true;
            }
