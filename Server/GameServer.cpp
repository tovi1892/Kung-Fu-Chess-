...
void GameServer::handleMessage(const ConnectionId& id, const std::string& text) {
    {
        ...
        if (sessions_.isPending(id)) {
            ...
            // Already in a room (or a rejected/unrecognized connection, in which case this
            // is a no-op - see ClickRouter::handleClick): the only message a joined
            // connection ever sends is a click.
            handleClick(id, decoded, roomManager_);
        }
    }
    outbox_.flush();
}
...
bool GameServer::tryReconnect(const ConnectionId& id, const std::string& username) {
    const auto reconnected = roomManager_.reconnect(username, id);
