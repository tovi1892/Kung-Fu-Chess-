...
PlayerColor Piece::color() const { return color_; }
Position Piece::position() const { return position_; }
PieceState Piece::state() const { return state_; }

void Piece::setPosition(const Position& position) { position_ = position; }
void Piece::setState(PieceState state) { state_ = state; }

bool Piece::isAirborne() const { return state_ == PieceState::Airborne; }
